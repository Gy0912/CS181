...
def sentence3() -> Expr:
    ...
    born0 = PropSymbolExpr('PacmanBorn', 0)
    killed0 = PropSymbolExpr('PacmanKilled', 0)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i in range (0, len(literals)):
        for j in range (i+1, len(literals)):
            list.append(disjoin(~literals[i], ~literals[j]))
    return conjoin(list)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    locations = [PropSymbolExpr(pacman_str, coord[0], coord[1], time=t) for coord in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(locations))
    dirlist = exactlyOne([PropSymbolExpr(direction, time = t) for direction in DIRECTIONS])
    pacphysics_sentences.append(dirlist)
    if(sensorModel != None):
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print(t)
        pacman_locations = exactlyOne([PropSymbolExpr(pacman_str, coord[0], coord[1], time = t) for coord in non_wall_coords])
...
def foodLogicPlan(problem) -> List:
    ...
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time = 0))
    for coords in food:
        KB.append(PropSymbolExpr(food_str, coords[0], coords[1], time = 0))
    for t in range(50):
        pacman_locations = exactlyOne([PropSymbolExpr(pacman_str, coord[0], coord[1], time = t) for coord in non_wall_coords])
        KB.append(pacman_locations)
        goal_state = [~PropSymbolExpr(food_str, coord[0], coord[1], time = t) for coord in food]
        model = findModel(conjoin(goal_state) & conjoin(KB))
        if (model):
            return extractActionSequence(model, actions)
        possible_actions = exactlyOne([PropSymbolExpr(action, time = t) for action in actions])
        KB.append(possible_actions)
...
def localization(problem, agent) -> Generator:
    ...
    for coords in all_coords:
        if (coords in walls_list):
            KB.append(PropSymbolExpr(wall_str, coords[0], coords[1]))
        else:
            KB.append(~PropSymbolExpr(wall_str, coords[0], coords[1]))
    for t in range(agent.num_timesteps):
        ...
        for coords in non_outer_wall_coords:
            if (findModel(conjoin(KB + [PropSymbolExpr(pacman_str, coords[0], coords[1], time = t)]))):
                possible_locations.append(coords)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))
        for coords in non_outer_wall_coords:
            if (entails(conjoin(KB), PropSymbolExpr(wall_str, coords[0], coords[1]))):
                KB.append(PropSymbolExpr(wall_str, coords[0], coords[1]))
                known_map[coords[0]][coords[1]] = 1
            elif (entails(conjoin(KB), ~PropSymbolExpr(wall_str, coords[0], coords[1]))):
                KB.append(~PropSymbolExpr(wall_str, coords[0], coords[1]))
                known_map[coords[0]][coords[1]] = 0
        agent.moveToNextState(agent.actions[t])
        yield known_map
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for coords in non_outer_wall_coords:
            if (findModel(conjoin(KB + [PropSymbolExpr(pacman_str, coords[0], coords[1], time = t)]))):
                possible_locations.append(coords)
            if entails(conjoin(KB), PropSymbolExpr(pacman_str, coords[0], coords[1], time = t)):
                KB.append(PropSymbolExpr(pacman_str,  coords[0], coords[1], time = t))
            elif entails(conjoin(KB), ~PropSymbolExpr(pacman_str, coords[0], coords[1], time = t)):
                KB.append(~PropSymbolExpr(pacman_str, coords[0], coords[1], time = t))
        agent.moveToNextState(agent.actions[t])
