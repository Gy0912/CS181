...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for assignment in finalfactor.getAllPossibleAssignmentDicts():
            probability = 0
            for value in variablesDomainDict[eliminationVariable]:
...
def normalize(factor: Factor):
    ...
    conditioned_variables = factor.conditionedVariables()
    variablesDomainDict = factor.variableDomainsDict()
    Sum = sum([factor.getProbability(assignment) for assignment in factor.getAllPossibleAssignmentDicts()])
    if Sum == 0:
        return None
    for variable in unconditioned_variables.copy():
        if len(variablesDomainDict[variable]) == 1:
            unconditioned_variables.remove(variable)
            conditioned_variables.add(variable)
    finalfactor = Factor(unconditioned_variables, conditioned_variables, variablesDomainDict)
    for assignment in finalfactor.getAllPossibleAssignmentDicts():
        probability = factor.getProbability(assignment)
        finalfactor.setProbability(assignment, probability / Sum)
    return finalfactor
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
